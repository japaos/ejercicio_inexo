...
//deteccion de boton de guardar paciente
    $("#almacenar").click(function(){
        let ed=$("#edad").val();   //edad del paciente
        let hi=$("#historia").val();                //historia clinica
        let no=$("#nombre").val();                  //nombre paciente
        let ex=$("#extra").val();
        if(!$.isNumeric(ex)||ed==''||hi==''||no==''|| !$.isNumeric(ed) || ed<0 ||ex==''||!$.isNumeric(hi)){      //validacion de campos llenados por el usuario   
